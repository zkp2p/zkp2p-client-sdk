...
export const isProvidedIdEqualToRegistration = async (input: string, registrationHash: string): Promise<boolean> => {
  ...
  const venmoHashFromInputNumber = BigInt(venmoHashFromInput);

  const existingRegistrationNumber = BigInt(registrationHash);

  return existingRegistrationNumber === venmoHashFromInputNumber;
}
...
function Bytes2Packed(n: number, inArr: number[]) {
  let index = 0;
  const out: [bigint, bigint, bigint] = [ZERO_BIGINT, ZERO_BIGINT, ZERO_BIGINT];
...
function Bytes2Packed8(n: number, inArr: number[]) {
  let index = 0;
  const out: [bigint, bigint, bigint, bigint, bigint, bigint, bigint, bigint] = [ZERO_BIGINT, ZERO_BIGINT, ZERO_BIGINT, ZERO_BIGINT, ZERO_BIGINT, ZERO_BIGINT, ZERO_BIGINT, ZERO_BIGINT];
