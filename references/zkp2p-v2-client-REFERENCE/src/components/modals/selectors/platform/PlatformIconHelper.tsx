...
export default function PlatformIconHelper({ keyType, name }: { keyType: 'mail' | 'browser'; name: string; }) {
  /*
   * State
   */
  ...
  const getIconForType = (key: 'mail' | 'browser'): React.ReactNode => {
    switch (key) {
      case 'mail':
        return <Mail size={22} color="#FFF" />;
      case 'browser':
        return <Globe size={22} color="#FFF" />;
      default:
