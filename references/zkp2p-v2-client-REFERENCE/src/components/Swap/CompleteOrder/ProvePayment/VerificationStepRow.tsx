...
import { Download, Cpu, Check, Circle, Play, Upload, RefreshCw, AlertTriangle, Send } from 'react-feather';
  ...
  const getLeftIcon = () => {
    switch (type) {
      case VerificationStepType.SELECTED_PAYMENT:
        return fiatCurrency ? 
          <FlagIcon className={`fi fi-${currencyInfo[fiatCurrency].countryCode}`} /> :
          <StyledSend progress={progress} />
      case VerificationStepType.DOWNLOAD:
      ...
      case VerificationStepType.SUBMIT:
        return (
          <IconStack>
            <StyledTokenIcon src={tokenInfo[usdcInfo.tokenId].icon} alt="USDC" />
            <ChainIconWrapper>
              <ChainIcon src={tokenInfo[usdcInfo.tokenId].chainIcon} alt="Base" />
            </ChainIconWrapper>
          </IconStack>
        );
      case VerificationStepType.SWAP:
        if (swapToken) {
          return (
            <IconStack>
              <StyledTokenIcon src={tokenInfo[swapToken]?.icon} alt={swapToken} />
              <ChainIconWrapper>
                <ChainIcon src={tokenInfo[swapToken]?.chainIcon} alt={tokenInfo[swapToken]?.chainName} />
              </ChainIconWrapper>
            </IconStack>
          );
        } else {
          return <StyledSwap progress={progress} />;
        }
      default:
        return null;
    }
    ...
    switch (type) {
      case VerificationStepType.SELECTED_PAYMENT:
        if (paymentDate && paymentPlatform) {
          return `${paymentDate} on ${paymentPlatformInfo[paymentPlatform].platformName}`;
        }
...
const StyledSend = styled(IconBase).attrs({ as: Send })``;
