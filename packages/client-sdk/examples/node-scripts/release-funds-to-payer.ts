/**
 * Example: Release funds to payer (Orchestrator v3)
 *
 * Env:
...
async function main() {
  ...
  const hash = await client.releaseFundsToPayer({ intentHash: INTENT_HASH });
  console.log('releaseFundsToPayer tx hash:', hash);
}
